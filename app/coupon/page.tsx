...
import { ArrowLeft, Percent, Users, Gift, Calendar, Home, ShoppingBag } from "lucide-react"
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#1D3F4F] via-[#2A5A6B] to-[#1D3F4F] pb-20">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-white mb-2 font-serif text-5xl font-medium">Coupon</h1>
        </div>
          ...
          {activeTab === "available" &&
            availableCoupons.map((coupon) => (
              ...
              >
                ...
                <div className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#ec8a4a] to-[#dc7867] mb-2">
                  {coupon.discount}
                </div>
                <p className="text-white/70 text-sm mb-4">{coupon.description}</p>

                <Button className="w-full bg-gradient-to-r from-[#ec8a4a] to-[#dc7867] hover:from-[#dc7867] hover:to-[#ec8a4a] text-white py-2 rounded-xl font-medium transition-all duration-200">
                  Use Coupon
                </Button>
              </div>
            ))}
            ...
            usedCoupons.map((coupon) => (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <span className="bg-white/20 text-white/70 px-3 py-1 rounded-full text-sm font-medium">
                    {coupon.status}
                  </span>
                </div>

                <h3 className="text-lg font-semibold text-white/80 mb-2">{coupon.title}</h3>
                <div className="text-2xl font-bold text-white/60 mb-2">{coupon.discount}</div>
                ...
                <Button
                  disabled
                  className="w-full bg-white/10 text-white/50 py-2 rounded-xl font-medium cursor-not-allowed"
                >
                  Already Used
                </Button>
              </div>
        ...
        <div className="bg-gradient-to-r from-[#2A5A6B]/50 to-[#1D3F4F]/50 backdrop-blur-sm border border-white/10 rounded-2xl p-8 text-center">
          ...
          <h3 className="text-white mb-2 font-serif font-medium text-3xl">Invite Friends & Earn</h3>
          <p className="text-white/70 mb-6">Refer friends and earn 10 points for each successful booking</p>
          <Button
            onClick={() => router.push("/refer-friend")} // Added navigation to refer-friend page
            className="bg-gradient-to-r from-[#ec8a4a] to-[#dc7867] hover:from-[#dc7867] hover:to-[#ec8a4a] text-white px-8 py-3 rounded-xl font-medium transition-all duration-200"
          >
            Refer a Friend
          </Button>
        </div>
